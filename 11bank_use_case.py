...
def calculate_late_payment(
        elements):  # [CT88330,Humberto,Banks,Serviceman,LN_1559,Medical Loan,26-01-2018,2000,30-01-2018]
...
def calculate_month(
        input_list):  # input  [CT88330,Humberto,Banks,Serviceman,LN_1559,Medical Loan,26-01-2018, 2000, 30-01-2018]
    ...
    input_list.append(
        str(payment_date.month))  # [CT88330,Humberto,Banks,Serviceman,LN_1559,Medical Loan,26-01-2018, 2000, 30-01-2018, 01]

    return input_list


def calculate_personal_loan_defaulter(
        input):  # input key -> CT68554,Ronald Chiki   value --> [01,05,06,07,08,09,10,11,12]
...
card_defaulter = (
        p
        | 'Read credit card data' >> beam.io.ReadFromText('input_data/bank/cards.txt', skip_header_lines=1)
...
medical_loan_defaulter = (
        p
        | beam.io.ReadFromText('input_data/bank/loan.txt',
                               skip_header_lines=1)  # 1stRow--> CT88330,Humberto,Banks,Serviceman,LN_1559,Medical Loan,26-01-2018, 2000, 30-01-2018
        ...
        | 'Make key value pairs' >> beam.Map(
    lambda elements: (elements[0] + ', ' + elements[1] + ' ' + elements[2], int(elements[9])))
...
personal_loan_defaulter = (
        p
        | 'Read' >> beam.io.ReadFromText('input_data/bank/loan.txt', skip_header_lines=1)
        ...
        | 'Make key value pairs loan' >> beam.Map(
    lambda elements: (elements[0] + ', ' + elements[1] + ' ' + elements[2], int(elements[9])))
...
both_defaulters = (
        ...
        | beam.CoGroupByKey()
        | 'Write p3 results' >> beam.io.WriteToText('output_data/class_bank/outputs/both')
)
